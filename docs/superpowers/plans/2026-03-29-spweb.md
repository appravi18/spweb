...
**Goal:** Build a full Next.js 16 website for secondprofits.net — a deep learning fintech company — with a dark terminal aesthetic, Jupyter-style code blog, Chart.js dashboard, and stylistic features (particle hero, ⌘K palette, typewriter, market ticker).

**Architecture:** Next.js 16 App Router with TypeScript; static generation for all blog/research/about/contact pages; client-side Chart.js for the dashboard. MDX files in `/content/blog/` are read at build time via `gray-matter` + `next-mdx-remote`. All styling via Tailwind CSS with custom design tokens.

**Tech Stack:** Next.js 16, TypeScript, Tailwind CSS, next-mdx-remote, gray-matter, KaTeX, Chart.js, JetBrains Mono + Inter (Google Fonts), Vercel, Vitest + React Testing Library
